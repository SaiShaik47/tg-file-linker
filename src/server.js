...
<head>
  ...
  <style>
    body{font-family:system-ui,Segoe UI,Roboto,Arial;display:flex;min-height:100vh;align-items:center;justify-content:center;margin:0;background:#0b0f17;color:#e9eef7}
    .card{width:92%;max-width:420px;background:#121a2a;border:1px solid #24314c;border-radius:16px;padding:20px;box-shadow:0 10px 30px rgba(0,0,0,.35)}
...
<body>
  <div class="card">
    ...
    <div class="hint">This link is protected.</div>
  </div>
</body>
</html>`;
}
...
<body>
  <div class="wrap">
    ...
    <div class="card">
      ...
      <p style="opacity:.75;font-size:13px;margin:12px 4px 0">Streaming page hosted on Railway. Video bytes served by Telegram CDN.</p>
    </div>
  </div>
</body>
</html>`;
...
async function getTelegramUrl(fileId) {
  return tg.getFileLink(fileId); // Telegram CDN URL
}

// STREAM PAGE (Railway page with player)
app.get("/s/:id", async (req, reply) => {
  try {
    ...
    if (row.password) {
      reply.type("text/html").send(passwordHtml(id, "s"));
      return;
    }

    reply.type("text/html").send(playerHtml(id, row.fileName));
  } catch (e) {
    reply.code(e.statusCode || 400).send({ ok: false, error: e.message });
  }
});

// VIDEO SOURCE (redirects to Telegram CDN)
app.get("/v/:id", async (req, reply) => {
  try {
    ...
    if (row.password) {
      reply.code(401).send({ ok: false, error: "Password required. Open the stream page first." });
      return;
    }
...
// DOWNLOAD (redirect + attachment header)
app.get("/d/:id", async (req, reply) => {
  try {
    ...
    if (row.password) {
      reply.type("text/html").send(passwordHtml(id, "d"));
      return;
    }
...
app.register(async function (f) {
  ...
  f.post("/p/s/:id", async (req, reply) => {
    try {
      ...
      if (!pass || pass !== row.password) {
        reply.code(401).type("text/html").send(passwordHtml(id, "s"));
        return;
      }

      // If password correct, show player page
      reply.type("text/html").send(playerHtml(id, row.fileName));
    } catch (e) {
      reply.code(e.statusCode || 400).send({ ok: false, error: e.message });
    }
  ...
  f.post("/p/d/:id", async (req, reply) => {
    try {
      ...
      if (!pass || pass !== row.password) {
        reply.code(401).type("text/html").send(passwordHtml(id, "d"));
        return;
      }
